...
def revenue_trend(df):
    ...
    freq = st.selectbox("Granularity", ['Monthly','Quaterly','Yearly'])
    if freq=='Monthly':
...
def Products_and_their_revenue(df):
    st.title('Products and their revenue')
...
def main():
    ...
    section = st.sidebar.selectbox('Choose dashboard group', [
        'Executive', 'Revenue ', 'Product & Inventory',
        'Operations', 'Advanced'
    ])
    ...
    if section=='Executive':
        ...
        page = st.sidebar.selectbox('Select', ['Products ', 'Brand Analytics ',  'Products and their revenue'])
        ...
        elif page.startswith('Products and their revenue'):
            Products_and_their_revenue(df)
